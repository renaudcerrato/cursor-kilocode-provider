...
export async function CursorPlugin(input: PluginInput): Promise<Hooks> {
  ...
  // loadModels can only read Kilo Code's durable store (auth.json /
  // KILO_AUTH_CONTENT); auth.loader gets live credentials via getAuth().
  ...
  async function resolveAccessToken(auth: Auth | StoredAuth): Promise<string | undefined> {
    ...
    if (auth.type === "oauth") {
      ...
      try {
        ...
        // Use the new token even if persisting back to Kilo Code fails.
        await persistAuthBestEffort({
          type: "oauth",
